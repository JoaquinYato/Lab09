public class GraphLink<E extends Comparable<E>>  {
    ...
    public Stack<E> Dijkstra(E v, E w) {
        ...
        ListLinked<Vertex<E>> noVisitados = new ListLinked<>(); // Esto será una copia de listVertex inicial
        ...
        while (nodoVert != null) {
            ...
            noVisitados.agregar(vert); // Llenar noVisitados con todos los vértices
        ...
        if (indexStart != -1) {
            ...
            // Esto no debería pasar si start se obtuvo de listVertex
            return ruta;
        }
        ...
        while (!noVisitados.inEmptyList()) {
            ...
            int posEnNoVisitados = 0; // Posición dentro de la lista noVisitados
            int minPosEnNoVisitados = -1;

            while (nodoNV != null) {
                Vertex<E> vert = noVisitados.getDato(nodoNV);
                int idxEnVerticesGlobal = indexOf(vertices, vert); // Índice en la lista global de vértices
                int dist = getDistancia(distancias, idxEnVerticesGlobal);
                if (dist < minDist) {
                    minDist = dist;
                    u = vert;
                    minPosEnNoVisitados = posEnNoVisitados;
                }
                nodoNV = noVisitados.getSiguiente(nodoNV);
                posEnNoVisitados++;
            }

            if (u == null) break; // No queda nada alcanzable

            // Remover u de noVisitados
            // removeAt(noVisitados, minPosEnNoVisitados); // Usamos removeNode que es más consistente
            noVisitados.removeNode(u);


            if (u.equals(end)) break; // Llegamos al destino
        ...
        while (paso != null) {
            ...
            if (idx == -1) break; // No se encontró en la lista de vértices (esto no debería pasar si llegó a 'end')
            paso = getPredecesor(predecesores, idx);
        }

        // La ruta está en orden inverso, la pila la invierte al sacarla.
        // Si queremos el camino de start a end, debemos invertir la pila o insertarla en otro orden.
        // Como la pila ya inserta en orden inverso, el pop la dará en orden correcto de fin a inicio.
        // Si se desea de inicio a fin, se puede transferir a otra pila o lista.
        return ruta;
    }
    ...
    private void setDistancia(ListLinked<Integer> distancias, int idx, int valor) {
        ...
        for (int i = 0; i < idx && nodo != null; i++) nodo = distancias.getSiguiente(nodo);
        if (nodo != null) {
    ...
    private int getDistancia(ListLinked<Integer> distancias, int idx) {
        // No es necesario la validación de rango con throw, el manejo de null es suficiente
        // para la lógica de Dijkstra si el índice es inválido (retorna valor por defecto, o se maneja fuera)
        ListLinked<Integer>.Nodo nodo = distancias.getPrimero();
        for (int i = 0; i < idx && nodo != null; i++) nodo = distancias.getSiguiente(nodo);
        if (nodo != null) {
            return distancias.getDato(nodo);
        }
        return Integer.MAX_VALUE; // o algún valor que indique no encontrado
    }
    private Vertex<E> getPredecesor(ListLinked<Vertex<E>> predecesores, int idx) {
        ...
        for (int i = 0; i < idx && nodo != null; i++) nodo = predecesores.getSiguiente(nodo);
        if (nodo != null) {
